<template>
  <div class="cognizance-box">
    <div class="leixing">{{this.videoTypeC}}</div>
  ...
  methods:{
    handler() {
      switch (this.$router.history.current.fullPath) {
        case '/video/entertainment': this.videoType = 'entertainment'
          this.videoTypeC = '娱乐视频'
          break;
        case '/video/sports': this.videoType = 'sports'
          this.videoTypeC = '体育视频'
          break;
        case '/video/music': this.videoType = 'music'
          this.videoTypeC = '音乐视频'
          break;
        case '/video/game': this.videoType = 'game'
          this.videoTypeC = '游戏视频'
          break;
        default:
          break;
      }

      this.request.get("http://localhost:9090/video/type/"+this.videoType).then(res=>{
